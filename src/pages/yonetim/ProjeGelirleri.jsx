              ...
              return (
                <tr key={m.id}>
                  ...
                  {Array.from({ length: maxStageCount }).map((_, i) => {
                    const s = stages[i]
                    return (
                      <td key={i} style={{ ...hucreStil, background: s ? renkArkaplan[renkler[i] || 'yok'] : 'transparent', minWidth: 100 }}>
                        {s?.id && (
                          <>
          ...
          {gorunenler.length > 0 && (
            <tfoot>
              <tr style={{ background: '#F8F7F2', fontWeight: 700 }}>
                ...
                {Array.from({ length: maxStageCount }).map((_, i) => <td key={i} style={hucreStil}></td>)}
      ...
      {duzenlenenId && (
        <div className="ekleme-kutusu">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '10px 0 4px' }}>
            <p style={{ fontSize: 12, fontWeight: 700, margin: 0 }}>Ödeme Aşamaları (kalan bakiyenin dağılımı)</p>
            <button onClick={asamaEkle} style={{ fontSize: 11, padding: '4px 8px', background: '#0F6E56', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>+ Aşama Ekle</button>
          </div>

          {taslakAsamalar.map((a, i) => (
            <div key={i} className="ekleme-satiri-2" style={{ marginBottom: 6, display: 'flex', gap: 6, alignItems: 'center' }}>
              <input
                type="text"
              ...
              />
              <button onClick={() => asamaSilTaslak(i)} style={{ background: '#D64545', color: '#fff', border: 'none', borderRadius: 4, width: 28, height: 28, cursor: 'pointer' }} title="Aşamayı sil">✕</button>
            </div>
          ))}

          <div className="ekleme-satiri-2" style={{ marginTop: 10 }}>
