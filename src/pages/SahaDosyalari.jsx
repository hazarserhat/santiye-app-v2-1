...
const ANA_KLASORLER = [
  { id: 'projeler', ad: 'Projeler ve Çizimler', ikon: '📐', renk: '#3B82F6', bg: '#EFF6FF' },
  { id: 'metrajlar', ad: 'Metraj ve Hesaplar', ikon: '🧮', renk: '#10B981', bg: '#ECFDF5' },
  { id: 'resmi', ad: 'Resmi Evraklar', ikon: '📝', renk: '#F59E0B', bg: '#FFFBEB' },
  { id: 'diger', ad: 'Diğer Saha Dosyaları', ikon: '📁', renk: '#6B7280', bg: '#F3F4F6' },
]
...
export default function SahaDosyalari() {
  ...
  const [aktifKlasor, setAktifKlasor] = useState(null) // null = Kök Dizin, 'projeler', vs.
  ...
  const dosyalariSistemeEkle = async (yeniDosyalar) => {
    ...
    const klasorAdi = ANA_KLASORLER.find(k => k.id === aktifKlasor)?.ad || aktifKlasor
    const driveFolderName = `SahaDosyalari/${santiyeAdi}/${klasorAdi}`
  ...
  const dosyaSil = async (dosya, e) => {
    ...
    try {
      const santiyeAdi = santiyeler.find(s => s.id === seciliSantiyeId)?.ad || 'Santiye'
      const klasorAdi = ANA_KLASORLER.find(k => k.id === aktifKlasor)?.ad || aktifKlasor
      await moveToSilinenler(dosya.dosya_url, `Silinenler/SahaDosyalari/${santiyeAdi}/${klasorAdi}`)
    ...
    <div className="sayfa">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 8, flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, fontSize: 22, fontWeight: 600, color: '#1D9596', letterSpacing: '-0.2px' }}>
          Saha Dosyaları (Drive)
        </h2>
      </div>

      <div style={{ background: '#f8f7f2', padding: '12px', borderRadius: 12, boxShadow: 'inset 0 2px 8px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.03)', marginBottom: 16 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: '#333' }}>
              / {ANA_KLASORLER.find(k => k.id === aktifKlasor)?.ad}
            </span>
          </div>
          ...
          >
            {yukleniyor ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                ...
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#1D9596' }}>Dosyalar Yükleniyor... Lütfen bekleyin.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 32 }}>📥</span>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#555' }}>
                  Dosyaları buraya sürükleyin veya <span style={{ color: '#1D9596', textDecoration: 'underline' }}>seçmek için tıklayın</span>
                </p>
